...
<div style="padding: 32px;">
    ...
    <div style="margin-bottom: 24px;">
        <a href="{{ route('admin.rooms.show', $room) }}" style="font-size: 12px; color: #C4A08A; text-decoration: none;">← Room {{ $room->room_number }}</a>
        <h1 style="font-size: 20px; font-weight: 500; color: #3D2314; margin-top: 4px;">Edit room {{ $room->room_number }}</h1>
        <p style="font-size: 13px; color: #7A5542; margin-top: 2px;">Update the details for this room.</p>
    </div>
        ...
        <div style="background: #fff; border-radius: 12px; border: 0.5px solid #E8DDD4; margin-bottom: 16px; overflow: hidden;">
            ...
            <div style="padding: 20px; display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
                ...
                <div>
                    ...
                    <input type="number" name="monthly_rate" value="{{ old('monthly_rate', $room->monthly_rate) }}" min="0" step="0.01"
                        style="width: 100%; border: 0.5px solid #E0D5CB; border-radius: 8px; padding: 8px 12px; font-size: 13px; color: #3D2314; background: #FDFAF7; box-sizing: border-box;">
                ...
                <div>
                    ...
                    <input type="number" name="max_occupants" value="{{ old('max_occupants', $room->max_occupants) }}" min="1"
                        style="width: 100%; border: 0.5px solid #E0D5CB; border-radius: 8px; padding: 8px 12px; font-size: 13px; color: #3D2314; background: #FDFAF7; box-sizing: border-box;">
        ...
        <div style="background: #fff; border-radius: 12px; border: 0.5px solid #E8DDD4; margin-bottom: 24px; overflow: hidden;">
            ...
            <div style="padding: 20px;">
                <textarea name="notes" rows="4" placeholder="Any special notes about this room (optional)..."
                        style="width: 100%; border: 0.5px solid #E0D5CB; border-radius: 8px; padding: 10px 12px; font-size: 13px; color: #3D2314; background: #FDFAF7; box-sizing: border-box; resize: none;">{{ old('notes', $room->notes) }}</textarea>
            </div>
        </div>
    ...
    <div style="margin-top: 16px;">
        <form action="{{ route('admin.rooms.destroy', $room) }}" method="POST"
            ...
            @csrf @method('DELETE')
            <button type="submit" style="font-size: 13px; color: #993C1D; background: none; border: none; cursor: pointer; padding: 0;">
                Delete room
            </button>
        </form>
    </div>
